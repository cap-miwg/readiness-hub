...
- `AUTH_MODE=dev` (local work, demos): user picker, no Google dependency. **Fail-closed mechanics:** the reference compose binds to 127.0.0.1 by default, the app refuses to start when dev mode is combined with configured Google credentials (misconfigured production), a persistent banner renders, and non-demo ingest is refused unless `DEV_ALLOW_REAL_INGEST=true` is set explicitly. (A NODE_ENV check is not usable here: the container image always runs NODE_ENV=production, including the sanctioned quickstart demo.)
